...
class DecomposedVAE:
    def __init__(self, train, valid, test, feat, bsz, save_path, logging, log_interval, num_epochs,
        ...
        self.dec_optimizer = optim.SGD(
            self.vae.decoder.parameters(), lr=self.dec_lr)

        self.nbatch = len(self.train_data)
        self.anneal_rate = (1.0 - kl_start) / (warm_up * self.nbatch)

    def _feed_batch(self, batch_id):
        batch_data = self.train_data[batch_id]
        batch_feat = self.train_feat[batch_id]
        ...
        while self.aggressive and sub_iter < 100:
            ...
            logits, kl1_loss, kl2_loss, reg_ic = self.vae.loss(
                batch_data_enc, batch_feat_enc)
            logits = logits.view(-1, logits.size(2))
            rec_loss = F.cross_entropy(
                logits, target_enc.view(-1), reduction="none")
            ...
            batch_data_enc = batch_data_enc[shift:min(
                burn_sent_len, shift + 10), :]
        ...
        if self.text_only:
            ...
            var_loss, reg_loss, var_raw_loss = self.vae.var_loss(
            ...
            srec_loss, reg_loss, srec_raw_loss = self.vae.var_loss(
    ...
    def train(self, epoch):
        ...
        for idx in np.random.permutation(range(self.nbatch)):
            loss, b_num_words, b_num_sents, b_total_rec_loss, b_total_kl1_loss, b_total_kl2_loss, b_total_srec_loss = self._feed_batch(
                idx)
            ...
            #         self.vae.decoder.parameters(), 5.0)
    ...
    def fit(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if self.aggressive:
                cur_mi = val_loss[4]
                self.logging("pre mi: %.4f, cur mi:%.4f" %
                             (self.pre_mi, cur_mi))
